...
int main(int argc,char**argv){
    ...
    const uint32_t W        = argc>4? (uint32_t)atoi(argv[4]) : 8;
    ...
    bool ok = rotor_bsgs_gpu::launch_giant_batch(startXY.data(), strideXY,
                                           nThreads, nSteps, W, out, err);
    if(!ok){ fprintf(stderr,"launch_giant_batch FAILED: %s\n", err.c_str()); return 2; }
    printf("launch_giant_batch OK: %u walks x %u steps (W=%u) on GPU\n", nThreads, nSteps, W);
